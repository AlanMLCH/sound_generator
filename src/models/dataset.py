...
def sample_sequence(model, vocab, steps=512, temperature=1.0, top_p=0.9, cond_instruments=None, device="cpu"):
    """
    Works with either LSTM (expects model(x_chunk, state)->(logits,state)) or
    Transformer (expects model(x)->logits).
    Uses nucleus (top-p) sampling.
    """
    import torch
    ...
    if cond_instruments:
        for i in cond_instruments:
            ...
            if tok is not None:
                seq.append(tok)
    # helpful priming for REMI-ish vocab (no harm if tokens missing)
    for t in ["BAR", "BEAT_1"]:
        if t in vocab.stoi:
            seq.append(vocab.stoi[t])

    x = torch.tensor(seq[-1024:] if seq else [0], dtype=torch.long, device=device).unsqueeze(0)
    state = None

    for _ in range(steps):
        # Try LSTM-style forward first (x_chunk,state) -> (logits,state)
        logits = None
        try:
            logits, state = model(x[:, -1:], state)  # LSTM path
        except TypeError:
            logits = model(x)                        # Transformer path
